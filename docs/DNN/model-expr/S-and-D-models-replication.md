...
最后值得一提的是，“选取IoU最大的框”这个操作似乎因为涉及到 `argmax` 操作而**不可导**，但是由于我们对不同的情况分配了不同的损失，因此我们事实上执行的操作是**对最大框**利用 $\mathcal{L}_1$ 回传梯度而对其他框利用 $\mathcal{L}_{\mathrm{N}}$ 回传梯度，这样整个网络就完全可导了。
