...
$$
\begin{align*}
    g_n&=\nabla\mathcal{L(\bm{x};\theta_{n-1})}\\
    M_{n}&=\beta_1M_{n-1}+\beta_3g_n\\
    \theta_n&=\theta_{n-1}-\eta M_n
\end{align*}
$$

（若不做特殊说明，$\nabla\mathcal{L(\bm{x};\theta_{n-1})}$ 一概指一个 mini-batch 的平均梯度即 $\dfrac{1}{|\mathcal{B}|}\sum^{|\mathcal{B}|}_{i=1}\nabla\mathcal{L}(x_i;\theta_{n-1})$）
...
入手（其中 $g'_n=\nabla\mathcal{L}(\bm{x};\theta_{n-1}-\eta \beta_1M_{n-1})$），为了让这个 $g'_n$ 能够较好地被分离出去，首先在左右两边配上一个 $-\eta \beta_1M_n$，有点类似于去找前瞻位置。然后：
...
$$
\begin{align*}
    g'_n&=\nabla\mathcal{L(\bm{x};\hat\theta_{n-1})}\\
...
SGDW 能够具有更快的收敛速率，尤其对于梯度不对称场景下，能够实现均衡的梯度累积，即减缓前后横跳，加速向下滚动。动量居功至伟。
...
$$
\begin{align*}
    g_{n} &= -\eta\nabla\left(\mathcal{L}(\bm{x};\theta_{n-1})+\dfrac{\lambda}{2}|\theta_{n-1}|^2\right)\\
    &=-\eta\nabla\mathcal{L}(\bm{x};\theta_{n-1})-\eta\lambda\theta_{n-1}\\
...
我们回顾一下怎么给一个多元函数 $f(\bm{x})$ 做二阶泰勒展开。

在一阶的情况下，很明显有

$$
f(\bm{x})\approx f(\bm{x_0})+\nabla f(\bm{x_0})\cdot (\bm{x-x_0})
$$
...
$$
f(\bm{x})\approx f(\bm{x_0})+\nabla f(\bm{x_0})(\bm{x-x_0})+\dfrac{1}{2}(\bm{x-x_0})^\top H f(\bm{x_0})(\bm{x-x_0})
$$
...
$$
f(\bm{x})=\dfrac 12\bm{x}^\top A\bm{x}+\bm{b}^\top \bm{x}
$$

那么在这个面上的任一点有梯度 $\nabla f=A\bm{x}-\bm{b}$ 以及海森矩阵 $Hf=A$。

令梯度等于 $0$，实质就是求解线性方程组 $H\bm{x}=\bm{b}$，为此我们定义残差 $\bm{r}=H\bm{x}-\bm{b}$
