...
<b>答案：A。</b>考虑一开始有 n 个独立的结点，从任意一个结点开始加边，我们尽量不要形成环，那么每一次加边都选择度为 0 的结点，加了 n - 1 次边之后我们发现没有度为 0 的结点了，也就是当 e = n - 1 时，连通图退化成树。此时从结点 i 到结点 j 再加一次边 e_n ，那么 i 到 j 原先的联通路径加上这个新加入的边刚刚好就构成一个环。
