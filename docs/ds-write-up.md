...
本题 \(s\) 是一个障眼法，决定循环次数的只有 \(i\) 和 \(n\)。根据代码，\(i\) 从 \(1\) 开始，每一次循环 \(i\) 都增加 \(1\)，直到 \(i\) 的值增加到 \(n\) 为止。因此一共执行了 \(n\) 次循环，选A。
...
$$
也就是 \(s\) 增长率在 \(x^2\) 量级，由于循环条件为 \(s\le n\)，就可以得出 \(x=O(\sqrt n)\)。
<p></p>
...
<b>答案：A。</b>首先复习完全二叉树的定义：完全二叉树是除了最后一层以外其他层都满结点的二叉树，且最后一层的结点连续集中在最左边。题目问至少，所以我们让最后一层只有一个结点，那么总的结点数为：
...
<b>答案：C。</b>首先，哈夫曼树只有叶子结点可以表示数据的编码，因此我们其实就是要求出这个哈夫曼树叶子结点的个数。回忆构建哈夫曼树的过程，一开始有单独的一个叶子结点，然后每添加一个数据就新增一个叶子结点用来表示这个数据，并新增一个非叶子结点。因此叶子结点个数始终比非叶子结点数多 1 。利用这个性质就可算出答案是 500。
...
<b>答案：A。</b>对于结点 \(i\) 和 \(j\) ，邻接矩阵里面的元素 \(G[i][j]\) 表示 \(i\) 到 \(j\) 的路径长度，同理元素 \(G[j][i]\) 表示 \(j\) 到 \(i\) 的路径长度。由于是一个无向图，所以两者应该相等。所以是对称矩阵。至于稀疏还是稠密，就取决于图本身了。
...
<b>答案：A。</b>考虑一开始有 \(n\) 个独立的结点，从任意一个结点开始加边，我们尽量不要形成环，那么每一次加边都选择度为 \(0\) 的结点，加了 \(n - 1\) 次边之后我们发现没有度为 \(0\) 的结点了，也就是当 \(e = n - 1\) 时，连通图退化成树。此时从结点 \(i\) 到结点 \(j\) 再加一次边 \(e_n\) ，那么 \(i\) 到 \(j\) 原先的连通路径加上这个新加入的边刚刚好就构成一个环。
...
<p></p>
(1) : 如果这个循环单链表只有尾结点，那么在进行插入和删除最后一个元素的时候，都要去寻找尾结点的前驱结点，但这又是一个单链表，没有前向信息，所以会浪费循环一次即 \(O(n)\) 的时间去找前驱。
<p></p>
(2) : 由于这个非循环的双链表没有头结点，在插入和删除第一个元素的时候都要横跨整个链表，也要花 \(O(n)\) 的时间。
<p></p>
(3) : 由于这是一个循环双链表，这时候相比于 (1) 和 (2) 而言，插入和删除都能够很方便（也就是 \(O(1)\) 时间）获取到头尾附近结点的地址，以进行修改。
<p></p>
(4) : 问题其实和 (1) 一样，即使有头结点也很难去找到尾结点的前驱结点。
<p></p>
...
<p></p>
构造反例时可以考虑构造一个树，然后将一个叶子结点和根结点相连，这条新边的权值小于原来根到这个叶子结点的路径长度即可，比如下面这个简单的反例：
<p></p>
...
<p></p>
注意先序遍历是 ULR 的顺序，所以第一个结点一定是根结点，又因为 BST 的左子树都是小于根结点，右子树大于根结点，因此找到第一个大于根结点的数据就可以区分开左右子树了，如图：
<p></p>
<a href="https://imgse.com/i/pEvVDQe"><img src="https://s21.ax1x.com/2025/05/16/pEvVDQe.png" alt="pEvVDQe.png" border="0" /></a>
<p></p>
对于查找成功而言，找到每个结点的查找长度就是到根结点的路径长度加一（因为要算上根结点本身的查找）：

$$
L = \dfrac{1}{10}(1+2\times 2+3\times 4+4\times 3)=2.9
$$
...
$$
L = \dfrac{1}{11}(3\times 5+4\times 6)=\dfrac{39}{11}
$$
...
<p></p>
思路是从根结点开始深搜，如果当前结点的子结点值为 x，那么该结点就是所求的结点 p。而且如果已经求得 p，那么剩下的搜索都可以剪枝跳过了。
<p></p>
...
<p></p>
<b>答案：A。</b>我们要做的就是让前后结点“绕过” p，也就是让 p 的前驱的后继设置成 p 的后继，同时 p 的后继的前驱设置成 p 的前驱。把这个操作翻译成代码就得到了 A 选项。
<p></p>
...
<p></p>
<b>答案：C</b>。除了根结点以外，所有结点都能被记作一个分支，因此是总结点数减去 1。
<p></p>
...
<p></p>
<b>答案：A</b>。本题考察左儿子右兄弟法。也就是右边的子树都和它的父母结点是同一层级的。右子树的结点数为 n，说明除了第一棵树以外所有树的结点个数为 n。
<p></p>
...
<b>答案：A</b>。
<p></p>
首先先序遍历是 ULR 的顺序，中序遍历是 LUR 的顺序，后序遍历是 LRU 的顺序。从中序序列里面找到先序系列的第一个结点，就自然地划分开了左右子树，由此我们可以构建这样一个二叉树：
...
<p></p>
<b>答案：A</b>。要求强连通，则每个结点至少要有一个入度和一个出度，这样才能保证其他结点能访问到它，它也能访问到其它结点。不难想到把这 n 个点串成一个循环单链表，这样每一个点的出度就是下一个结点的入度，一共 n 条边。若比 n 更小，必然存在有结点没有入度或者出度。
<p></p>
...
<p></p>
<b>答案：C</b>。Dijkstra 算法分两步，对每个结点而言，第一步求取最短路径边，第二步松弛，我们分开来看。
<p></p>
邻接矩阵是一个很糟糕的数据结构。在松弛找邻接边的时候就要遍历所有结点，时间复杂度 O(n)，所以第二步时间复杂度就来到了 O(n^2)。第一步我们可以使用链表做到总 O(n^2) 的时间复杂度或者利用堆实现更优的时间复杂度，但都无力回天，总的时间复杂度为 O(n^2)。
<p></p>
...
<p></p>
<b>答案：D</b>。平衡因子是左子树高度减去右子树高度，由于平衡因子是0，意味着这是一颗满二叉树。因此结点个数：
...
<p></p>
我们考虑最朴素的构造：所有结点从根结点开始串成一条链，然后对于度为 k 的结点，补充 k - 1 个叶子结点，就得到了上式。接下来调整树的结构，可以发现无论怎么调整，只要满足题目条件，都不会改变叶子结点个数。
<p></p>
...
<p></p>
<b>答案：B</b>。首先根结点有两个空指针域，然后每次合并时都增加一个新的根结点和新的叶子结点，叶子结点又有两个空指针域，因此是 2m。
<p></p>
...
<p></p>
<b>答案：A</b>。首先新结点的关键字更小，因此插在左子树。因为一开始没有找到，所以是连在某个叶子结点的空指针域上面，形成新的叶子结点。
...
<p></p>
由于有旋转操作，因此无法保证插入的结点最后一定是叶子结点，但是一定是在左子树。
<p></p>
...
<p></p>
<b>答案：A</b>。前序：ULR，中序LUR，后序LRU，可见前序和中序都把右子树放在最后遍历。为了要让最后一个结点一致，必须保证右子树存在，因此这个结点一定是叶子。
<p></p>
...
<p></p>
<b>答案：A或者C</b> BST 不是平衡的。或者考虑：B+树是索引数据结构，但其他选项不是。
<p></p>
...
<p></p>
<b>答案：D</b>。考虑 rear 指向的是最后一个元素而不是最后一个元素的后一个元素，所以应该加 1.
<p></p>
...
<p></p>
<b>答案：D</b>。先序线索在回溯的时候自动加上了后继的线索，中序树在回溯时既有前驱又有后继的线索，只有后序线索树加上前驱的线索而难以求得后继的线索。
<p></p>
...
将一棵树转换为二叉树时，采用“左孩子右兄弟”规则。原树中每个非叶子结点的子结点转换为二叉树后，第一个子结点作为左孩子，其余子结点依次作为前一个兄弟的右孩子。因此，每个非叶子结点若有 \( k \) 个子结点，则转换后会产生 \( k-1 \) 个右孩子结点。

原树共有 \( m \) 个叶子结点和 \( n \) 个非叶子结点，总边数为 \( m + n - 1 \)。所有非叶子结点的子结点数目之和等于总边数，即 \( \sum k_i = m + n - 1 \)。每个非叶子结点贡献的右孩子数目为 \( k_i - 1 \)，因此总共有：
\[
...
如上，将指针 1 调整为指针 1'，指针 2 调整为指针 4'，并加入指针 3' 和 2'，共 4 次调整。
...
考虑一个二叉树每加一个节点就高一层，根节点的左右结点分别是原来的根节点和新加入的结点，此时一共 \(2h-1\) 个结点。
<p></p>
...
题目没有说是带权的，因此认为是不带权的无向图。每加一次边 \(e(i,j)\) 都会设置 \( G[i][j] \) 和 \( G[j][i] \) 为 \(1\)，因此要减去\(2e\)。 
<p></p>
...
单链表上面难以进行基于下标的随机访问，也难以进行反向遍历，因此大量依赖这两个操作的排序算法都是不利于在单链表上面实现的。

希尔排序基于相隔 \(d\) 的分组，每一次往后面跳\(d\)个数据，涉及随机访问。

快速排序中的划分操作涉及反向遍历。

堆排序的调整操作，计算父节点过程中需要对下标进行随机访问。
<p></p>
...
对于顺序栈，只需要将 top 指针减一即可；对于链栈，只需要删除最后一个元素即可。
...
算法的运行时间不仅受到时间复杂度的影响，还受到常数的影响，考虑算法 A 对于规模 \(n\) 的问题需要执行 \(200 n\) 次操作而算法 B 对于同样规模的问题需要执行 \(2^n\) 次操作，则当 \(n < 12\) 时反而算法 B 的运行时间更短。
...
编码长度不一定满足带权路径最短这个条件，导致构造出来的编码不是哈夫曼编码。
...
考虑根节点到 BFS 生成树最深的一个节点的路径，则在 DFS 过程中，该节点必然是优先访问的，因此 DFS 生成树高度必然大于等于 BFS 生成树高度。
<p></p>
...
参考：https://oi-wiki.org/ds/binary-heap/#%E6%96%B9%E6%B3%95%E4%BA%8C%E4%BD%BF%E7%94%A8%E5%90%91%E4%B8%8B%E8%B0%83%E6%95%B4
<p></p>
...
<b>对。</b> 首先连通分量是连通的，其次连通分量的结点一定包含在图的所有结点里面，所以是连通子图。最后由于不能再往连通分量里面再添加边，因此是极大的连通子图。
...
考虑计算这个二叉树的总结点数：总结点数 = 度为2的节点数 * 2 + 度为 1 的节点数 + 1（根节点），可见是一个奇数。因此不能构造。
<p></p>
...
<b>错。</b>
教材8.5.3。经过结点 k-1 的最短路长度可能被引入结点 k 更新。
<p></p>
